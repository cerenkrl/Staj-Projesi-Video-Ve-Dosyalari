...
namespace Kutuphane_Otomasyon_Sistemi
{
    public partial class FormDuzenle : Form
    {
        ...
        void tableLoad()
        {
            ...
            {
                dgv.DataSource = IDataBase.DataToDataTable("select * from " + getTableName());
        ...
        private void dgv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                rowId = Convert.ToInt32(dgv.Rows[e.RowIndex].Cells["id"].Value);
                txtAd.Text = dgv.Rows[e.RowIndex].Cells["adi"].Value.ToString();
            }
        }
